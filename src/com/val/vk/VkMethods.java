...
import com.val.vk.exceptions.VkMethodResponseErrorException;
...
public class VkMethods {
    ...
    public UserList getUsers(int ids[]) throws EmptyParamException {
        ...
        if (ids.length == 0){
            throw new EmptyParamException("ids[] is empty");
        }
        ...
        try {
            response = new Gson().fromJson(httpsGetter.get(url), UserList.class);
            checkForErrors(response);
        } catch (IOException e) {
            e.printStackTrace();
        } catch (VkMethodResponseErrorException e){
            e.printStackTrace();
            System.exit(1);
        }
    ...
    private void checkForErrors(UserList response) throws VkMethodResponseErrorException {
        if (response.error!=null){
            throw new VkMethodResponseErrorException(response.error.error_msg);
        }
    }
